-- ============================================================================
-- 030 - Idempotencia, ancla de cuota y conflictos a revision
-- ============================================================================
-- FASE 0 del modo offline. Se puede correr y publicar por si solo: arregla un
-- problema que YA existe hoy, sin necesidad de nada offline.
--
-- PROBLEMA 1 — Cobro doble por reintento de red
-- lib/api-helper.ts reintenta hasta 3 veces ante errores de red y ningun pago
-- lleva identificador unico. Si el pago llega al servidor y se guarda pero la
-- respuesta se pierde por mala senal (el escenario tipico del cobrador en
-- campo), el reintento lo aplica OTRA VEZ y la base no tiene como detectarlo.
--   -> Solucion: llave de idempotencia generada en el dispositivo.
--
-- PROBLEMA 2 — El pago no dice a que cuota va
-- registrar_pago_atomico elegia la cuota con
--     WHERE estado='pendiente' ORDER BY numero_cuota ASC LIMIT n
-- es decir, "la mas antigua pendiente EN ESTE MOMENTO". Hoy funciona porque
-- pasan milisegundos desde que el cobrador ve la cuota. Con una cola offline
-- pasan horas y el pago caeria en otra cuota — o, si ya no queda ninguna
-- pendiente, el dinero se pierde devolviendo ok:true.
--   -> Solucion: el cliente manda payment_plan_id (ya lo tiene) y se cobra ESA.
--
-- PROBLEMA 3 — Los conflictos destruyen el registro del cobro
-- Si el prestamo se cancelo, la funcion hacia RAISE EXCEPTION y se perdia todo
-- rastro de que el cobrador recibio la plata.
--   -> Solucion: el conflicto entra a solicitudes_revision (la cola que
--      secretaria ya usa) en vez de fallar.
--
-- PROBLEMA 4 — Efectos secundarios fuera de la transaccion
-- El cobro de multa y la prorroga de americano corrian como escrituras
-- sueltas alrededor de la RPC. Si el pago entraba y la multa fallaba, la
-- multa quedaba cobrada y el dinero NUNCA entraba a la caja: un hueco
-- silencioso permanente.
--   -> Solucion: ambos se absorben dentro de la transaccion de la RPC.
--
-- Compatibilidad: todos los campos nuevos del payload son opcionales. Si no
-- vienen, el comportamiento es el de 028.
-- ============================================================================

-- ============================================================================
-- 1) Registro de operaciones ya procesadas (llave de idempotencia)
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.operaciones_procesadas (
  id          UUID        PRIMARY KEY,   -- generada en el dispositivo
  tipo        TEXT        NOT NULL,
  user_id     BIGINT,
  ruta_id     BIGINT,
...
CREATE INDEX IF NOT EXISTS idx_operaciones_procesadas_created
  ON public.operaciones_procesadas (created_at DESC);

ALTER TABLE public.operaciones_procesadas DISABLE ROW LEVEL SECURITY;

-- ============================================================================
-- 2) registrar_pago_atomico (v4, sobre 028)
-- ============================================================================
...
DECLARE
  ...
  v_idem            uuid;
  v_pp_id           uuid;      -- cuota objetivo elegida por el cobrador
  v_multa_id        uuid;      -- multa a cobrar junto con el pago
  v_metodo_pago     text;
  v_cliente_nombre  text;
  v_extender_cuotas int;       -- prorroga de americano
...
BEGIN
  ...
  --
  -- Si la operacion ya se proceso, devolvemos el MISMO resultado sin volver a
  -- tocar nada. Como toda la funcion corre en una transaccion, este INSERT
  -- bloquea la fila: dos envios simultaneos con la misma llave se serializan y
  -- el segundo entra por la rama del duplicado.
  -- --------------------------------------------------------------------------
  ...
  -- --------------------------------------------------------------------------
  -- 4) CONFLICTOS -> REVISION DE SECRETARIA
  --
  -- Antes esto era RAISE EXCEPTION y se perdia el registro de que el cobrador
  -- recibio la plata. Ahora la operacion entra a solicitudes_revision con el
  -- payload intacto y secretaria decide. Se registra el resultado bajo la
  -- llave de idempotencia para que un reintento no vuelva a encolarla.
  -- --------------------------------------------------------------------------
  v_motivo := NULL;
  ...
  ELSIF v_pp_id IS NOT NULL AND NOT v_asociar_a_hoy
    ...
    -- Ancla de cuota: se cobra la cuota que el cobrador vio, no la que el
    -- servidor elija.
  ...
  -- --------------------------------------------------------------------------
  -- 5) PRORROGA DE AMERICANO (antes eran 2 escrituras sueltas ANTES del pago,
  --    cuyo orden el codigo marcaba como CRITICO. Ahora va en la misma
  --    transaccion, asi que el orden esta garantizado por construccion.)
  -- --------------------------------------------------------------------------
  ...
  IF v_tipo = 'no_pago' THEN
    ...
    ELSE
      -- Con ancla: la cuota que vio el cobrador. Sin ancla: la del dia o la
      -- mas antigua vencida (comportamiento de 028).
      ...
      IF FOUND THEN
        UPDATE payment_plan
           SET estado = 'no_pago', fecha_pago_real = v_fecha_pago_real, monto_pagado = 0,
  ...
  ELSIF v_tipo IN ('pago_normal', 'pago_parcial') THEN
    ...
    ELSIF v_pp_id IS NOT NULL THEN
      -- ANCLA: se cobra exactamente la cuota que vio el cobrador. Ya quedo
      -- validada como 'pendiente' y bloqueada en el paso 4.
      ...
      -- Si el cobrador pago varias cuotas de una vez, las siguientes se toman
      -- en orden desde la anclada.
    ...
    ELSE
      -- Sin ancla: comportamiento de 028 (compatibilidad con llamadas viejas).
    ...
    -- Cancelacion automatica / cuota adicional (igual que 028)
  ...
  -- --------------------------------------------------------------------------
  -- 7) COBRO DE MULTA (antes corria FUERA de la transaccion: si el pago
  --    entraba y el ingreso fallaba, la multa quedaba cobrada y el dinero
  --    nunca llegaba a la caja. Ahora entran juntos o no entra ninguno.)
  -- --------------------------------------------------------------------------
  IF v_multa_id IS NOT NULL THEN
    ...
    IF v_multa_upd > 0 THEN
      INSERT INTO gastosregistros (
        ...
        NULL, v_multa_valor, 'Pago de multa por mora',
        NULL, 'Ingreso', 'NA', 'NA'
      );
  ...
  -- --------------------------------------------------------------------------
  -- 9) RESPUESTA (se guarda bajo la llave para que un reintento la repita)
  -- --------------------------------------------------------------------------
  v_resultado := jsonb_build_object(
    'ok',                            true,
...
-- ============================================================================
-- 3) aprobar_solicitud_revision — conservar los flags del abono
-- ============================================================================
-- Bug encontrado en la auditoria: al aprobar un abono se reenviaba el payload
-- guardado, pero el payload que arma el cliente NO incluia
-- generar_cuota_si_debe ni asociar_a_hoy. La funcion los tomaba como false y
-- podia cancelar un prestamo con deuda viva. Ahora ademas se limpia la llave
-- de idempotencia para que la aprobacion no choque con la operacion original.
-- ============================================================================
...
BEGIN
  ...
  SELECT id, tipo, ruta_id, payload, estado INTO v_row
    FROM solicitudes_revision WHERE id = v_solicitud_id FOR UPDATE;
  ...
  IF v_row.tipo = 'gasto' THEN
    RAISE EXCEPTION 'Las solicitudes de tipo gasto se aprueban desde el cliente';
  ...
  ELSIF v_row.tipo = 'abono' THEN
    -- Se quita idempotency_key: la operacion original pudo haberla registrado
    -- al enviarse a revision, y reusarla haria que esta aprobacion se tome
    -- como duplicado y no aplique nada.
    v_abono := (v_row.payload->'p_payload') - 'idempotency_key';
