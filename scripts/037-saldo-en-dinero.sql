-- ============================================================================
-- 037 - El saldo del prestamo se mide en dinero, no en capital
-- ============================================================================
-- Correr DESPUES del 033. Redefine registrar_pago_atomico (v7).
--
-- PROBLEMA
-- `loans.saldo` arranca en `valor_a_pagar` (capital + intereses) pero cada
-- pago le restaba solo el CAPITAL de las cuotas saldadas, no el dinero que
-- entrego el cliente.
--
-- Ejemplo real de la ruta 1 — don fausto: prestamo de 200, valor a pagar
-- 240, 24 cuotas de 10. Pago 3 cuotas, o sea 30 pesos.
--   · Lo que debe de verdad:      240 - 30 = 210
--   · Lo que guardaba loans.saldo: 240 - 25 = 215.01
-- porque las 3 cuotas llevan 25 de capital y 5 de interes.
--
-- La diferencia crece con cada abono. Si don fausto paga sus 24 cuotas
-- completas habra entregado 240, pero el saldo guardado solo habra bajado
-- 200 y quedara en 40. Ahi la funcion ve que no quedan cuotas pendientes,
-- marca el prestamo como CANCELADO... con 40 pesos colgando para siempre.
--
-- Son los prestamos que la auditoria encontro cerrados con deuda viva
-- (kevin 30, jhoana 217, diana 212, mery 163).
--
-- SOLUCION
-- El saldo se recalcula sobre el plan completo:
--     saldo = max(0, valor_a_pagar - suma de todo lo pagado)
-- Es la misma definicion que ya usa `ajustar_cuota_control_pagos` (script
-- 034) y la que muestra la vista `saldo_prestamos_clientes`, o sea la que
-- ve el cobrador en pantalla. Hasta hoy convivian dos numeros distintos
-- para lo mismo.
--
-- Se recalcula sobre el plan ENTERO y no restando de a poco: asi el
-- resultado es el mismo sin importar por que camino se aplico el pago, y no
-- se desvia abono tras abono.
--
-- CAMBIA CUANTO SE COBRA EN UNA CANCELACION TOTAL
-- Esa operacion le cobra al cliente `loans.saldo`. Con el numero viejo,
-- inflado, se le cobraba de mas: don fausto pagaria 215.01 cuando debe 210.
-- Desde ahora se le cobra lo que realmente falta. Los clientes que cancelan
-- anticipadamente pagan menos que antes — esa es la correccion.
--
-- NO reescribe el saldo de los prestamos que ya existen: cada uno se corrige
-- solo en su proximo cobro, cuando la funcion lo recalcule. Los que no
-- vuelvan a recibir un pago se pueden realinear de una con el PASO 5 de
-- scripts/corregir-valor-a-pagar.sql.
-- ============================================================================
...
DECLARE
  ...
  -- v7: el saldo se mide en DINERO, no en capital. Ver cabecera.
...
BEGIN
  ...
  SELECT saldo, estado, client_id, COALESCE(valor_a_pagar, valor)
    INTO v_saldo_actual, v_loan_estado, v_client_id, v_valor_a_pagar
    FROM loans
   WHERE id = v_loan_id
   FOR UPDATE;
  ...
  -- Lo que el cliente debe de verdad: el total con intereses menos todo
  -- lo que ha entregado. `loans.saldo` no sirve para esto porque se venia
  -- descontando por CAPITAL (ver cabecera del script).
  SELECT COALESCE(sum(monto_pagado), 0) INTO v_pagado_antes
    FROM payment_plan WHERE loan_id = v_loan_id;
  v_saldo_real := GREATEST(0, v_valor_a_pagar - v_pagado_antes);

  v_nuevo_saldo := v_saldo_real;
  ...
  IF v_motivo IS NOT NULL THEN
    ...
    v_resultado := jsonb_build_object(
      ...
      'cuotas_actualizadas', 0, 'nuevo_saldo', v_saldo_real,
  ...
  IF v_tipo = 'no_pago' THEN
    IF v_asociar_a_hoy THEN
      ...
      INSERT INTO payment_plan (
        ...
        v_saldo_real, 'no_pago', 0, v_fecha_pago_real, v_latitud, v_longitud, v_ruta_fila, true
      );
      v_cuotas_upd      := 1;
  ...
  ELSIF v_tipo = 'cancelacion_total' THEN
    ...
    LOOP
      IF v_cuotas_upd = 0 THEN
        UPDATE payment_plan
           SET estado = 'cancelada', monto_pagado = v_saldo_real, fecha_pago = v_fecha_pago,
  ...
  ELSIF v_tipo IN ('pago_normal', 'pago_parcial') THEN
    IF v_asociar_a_hoy THEN
      ...
      INSERT INTO payment_plan (
        ...
        GREATEST(0, v_saldo_real - v_monto),
      ...
      v_cuotas_upd      := 1;
      v_fila_hoy_creada := true;
      SELECT GREATEST(0, v_valor_a_pagar - COALESCE(sum(monto_pagado), 0))
        INTO v_nuevo_saldo
        FROM payment_plan WHERE loan_id = v_loan_id;
