...
export function SecretaryAuthorizations() {
  const [transactions, setTransactions] = useState<ApprovedTransaction[]>([])
  const [filteredTransactions, setFilteredTransactions] = useState<ApprovedTransaction[]>([])
  ...
  const fetchApprovedTransactions = async () => {
    try {
      ...
      // Fetch all transactions that have been approved by admin and are pending secretary approval
      const { data: transactionsData, error: transactionsError } = await supabase
        .from("gastosregistros")
        .select("id, fechahorasol, concepto, valor, tipo, ruta, limite, observacion, foto, adminid")
        .eq("estadoadmin", "aprobado")
      ...
      // Fetch admin data
      const { data: adminsData } = await supabase.from("admin").select("id, nombre, pais")
      ...
      // Map transactions with admin and country info
      ...
      console.error("[v0] Error in fetchApprovedTransactions:", error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      const { data: rutasData } = await supabase.from("rutas").select("id").order("id")
      if (rutasData) {
        const rutasList = rutasData.map((r: any) => ({ id: r.id, name: r.id.toString() }))
        setRutas(rutasList)
      }

      const { data: adminsData } = await supabase.from("admin").select("id, nombre").order("nombre")
      if (adminsData) {
        setAdmins(adminsData.map((a: any) => ({ id: a.id, name: a.nombre })))
      }

      const { data: countriesData } = await supabase.from("admin").select("pais").order("pais")
      if (countriesData) {
  ...
  const handleApprove = async (transactionId: number) => {
    ...
    try {
      const result = await approveTransactionSecretary({
        id: transactionId,
        status: "aprobado",
        secretaryName: userEmail,
      })
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl md:text-2xl font-bold text-card-foreground">Autorizaciones Secretaria</h2>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base md:text-lg">Registros Aprobados por Administrador</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {loading ? (
              ...
              <div className="text-center text-muted-foreground text-[12px] md:text-sm py-8">
                No hay transacciones pendientes de aprobación de secretaria
              </div>
            ) : (
